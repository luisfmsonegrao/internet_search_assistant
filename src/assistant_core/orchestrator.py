...
def orchestrate(query, session_id):
    ...
    history = load_interactions(HISTORY_CONTEXT_SIZE)  # Load recent interacion context. ToDo: consider adding relevance-based historic context
    ...
    if task_class == "CONTEXT_SEARCH":  # Answer from conversation context and knowledge base
    ...
    elif task_class == "WEB_SEARCH": # Agent waits for document ingestion before answering, this takes too long. In the future, should answer first based on summary, then ingest.
        ...
        for doc in results:
            upload_to_knowledge_base(doc) # consider limiting size of documents, or summarizing first, to keep DB size manageable
